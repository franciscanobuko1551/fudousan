...
const newConsultationButton = document.querySelector("#new-consultation");
...
function setActionButtons() {
  const hasConsultation = Boolean(currentConsultation);

  followUpButtons.forEach((button) => {
    button.disabled = !hasConsultation;
  });
  copyAnswerButton.disabled = !hasConsultation;
  newConsultationButton.disabled = !hasConsultation;
}
...
async function sendConsultation({ genre, message, mode = "initial", saveToHistory = true }) {
  setLoading(true);
  ...
  try {
    ...
    currentConsultation = { genre, message, reply, saveToHistory };
    showAnswer(saveToHistory ? reply : `${reply}\n\n※ この相談は履歴に保存していません。`);
...
form.addEventListener("submit", async (event) => {
  ...
  const saveToHistory = saveHistoryInput.checked;
...
followUpButtons.forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    await sendConsultation({
      ...
      saveToHistory: currentConsultation.saveToHistory,
    });
  });
});
...
copyAnswerButton.addEventListener("click", async () => {
  ...
  const originalText = copyAnswerButton.textContent;

  try {
    await navigator.clipboard.writeText(currentConsultation.reply);
    copyAnswerButton.textContent = "コピーしました";
  } catch {
    copyAnswerButton.textContent = "コピーできませんでした";
  } finally {
    window.setTimeout(() => {
      copyAnswerButton.textContent = originalText;
    }, 1800);
  }
});
